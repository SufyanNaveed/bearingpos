    ...
    <tr class="heading">
        ...
        foreach ($products as $row) {
            ...
            echo '<tr class="item' . $flag . '"> 
                            ...
                            <td style="font-size:14px; padding:0pt !important">' . $row['product'] . '  </td>
							<td style="width:12%;font-size:14px; padding:0pt !important">' . amountExchange($row['price'], $invoice['multi'], $invoice['loc']) . '</td>
                            <td style="width:12%;font-size:14px; padding:0pt !important" >' . +$row['qty'] . $row['unit'] . '</td>   ';
            if ($invoice['tax'] > 0) {
                $cols++;
                echo '<td style="width:16%;font-size:14px; padding:0pt !important">' . amountExchange($row['totaltax'], $invoice['multi'], $invoice['loc']) . ' <span class="tax">(' . amountFormat_s($row['tax']) . '%)</span></td>';
            }
            if ($invoice['discount'] > 0) {
                $cols++;
                echo ' <td style="width:16%;font-size:14px; padding:0pt !important">' . amountExchange($row['totaldiscount'], $invoice['multi'], $invoice['loc']) . '</td>';
            }
            echo '<td class="t_center" style="font-size:14px; padding:0pt !important">' . amountExchange($row['subtotal'], $invoice['multi'], $invoice['loc']) . '</td>
                        </tr>';
            if ($row['product_des']) {
                $cc = $cols++;
                echo '<tr class="item' . $flag . ' descr"> 
                            <td colspan="' . $cc . '" style="font-size:14px; padding:0pt !important">' . $row['product_des'] . '<br>&nbsp;</td>
