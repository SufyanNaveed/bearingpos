...
//search product in invoice
    ...
    public function puchase_search()
    {
        ...
        if ($name) {
            $query = $this->db->query("SELECT pid,product_name,product_code,fproduct_price,taxrate,disrate,product_des,unit FROM geopos_products WHERE " . $qw . "UPPER(product_name) LIKE '%" . strtoupper($name) . "%' OR UPPER(product_code) LIKE '" . strtoupper($name) . "%' LIMIT 60");
    ...
    public function pos_search()
    {
        ...
        foreach ($result as $row) {
        $supplierDetails = $this->products->supplier_ware($row['pid']);
        $supplierName = $supplierDetails['supplier'];
        $purchasePrice = $supplierDetails['purchaseprice'];
        if($supplierName){
            $supplierDetails = '<small style="white-space: pre-wrap;"><label class="label bg-blue" style="color:white;padding:3px">' . $supplierName . '</label></small>';
        }else{
            $supplierDetails = '';
        }

            $out .= '    <div class="col-3 border mb-1 "><div class="rounded">
                                 <a   id="posp' . $i . '"  class="select_pos_item btn btn-outline-light-blue round" data-fprice="' . $row['fproduct_price'] . '" data-supplier="' . $supplierName . '" data-purchaseprice="' . $purchasePrice . '" data-location="' . $row['product_location'] . '" data-name="' . $row['product_name'] . '"  data-price=" ' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . $row['taxrate'] . '"  data-discount="' . $row['disrate'] . '"   data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . $row['qty'] . '" data-unit="' . $row['unit'] . '" >
                                        <img class="round"
    ...
    public function v2_pos_search()
    {
        ...
        foreach ($result as $row) {
            ...
            $out .= '    <div class="col-2 border mb-1"  ><div class=" rounded" >
                                 <a  id="posp' . $i . '"  class="v2_select_pos_item round"   data-name="' . $row['product_name'] . '"  data-price="' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '"  data-tax="' . $row['taxrate'] . '"  data-discount="' . $row['disrate'] . '" data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . $row['qty'] . '" data-unit="' . $row['unit'] . '" >
                                        <img class="round"
                                             src="' . base_url('userfiles/product/' . $row['image']) . '"  style="max-height: 100%;max-width: 100%">
