...
//search product in invoice
    ...
    public function puchase_search()
    {
        ...
        if ($name) {
            $query = $this->db->query("SELECT pid,product_name,product_code,fproduct_price,taxrate,disrate,product_des,unit FROM geopos_products WHERE " . $qw . "UPPER(product_name) LIKE '%" . strtoupper($name) . "%' OR UPPER(product_code) LIKE '" . strtoupper($name) . "%' LIMIT 6");
