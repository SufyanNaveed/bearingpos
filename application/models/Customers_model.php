...
//SELECT geopos_customers.name, SUM(geopos_invoices.total) AS ttl FROM geopos_invoices LEFT JOIN geopos_customers ON geopos_customers.id=geopos_invoices.csd WHERE geopos_invoices.status!='paid' GROUP BY geopos_invoices.csd;
              $this->db->select('geopos_customers.*,SUM(geopos_invoices.total) AS total,SUM(pamnt) AS pamnt');
