...
class Dashboard_model extends CI_Model
{
    public function todayInvoice($today)
    {
        ...
        if ($this->aauth->get_user()->loc) $this->db->where('loc', $this->aauth->get_user()->loc);
    ...
    {
        $whr = '';
        if ($this->aauth->get_user()->loc) $whr = ' AND (geopos_warehouse.loc=' . $this->aauth->get_user()->loc . ')';
        $query = $this->db->query("SELECT geopos_products.* FROM geopos_products LEFT JOIN geopos_warehouse ON geopos_products.warehouse=geopos_warehouse.id  WHERE (geopos_products.qty<=geopos_products.alert) $whr ORDER BY geopos_products.product_name ASC LIMIT 10");
        return $query->result_array();
    }
    ...
    public function todayProfit($today)
    {
        ...
        if ($this->aauth->get_user()->loc) $this->db->where('geopos_invoices.loc', $this->aauth->get_user()->loc);
        $query = $this->db->get();
    ...
    {
        $whr = '';
        if ($this->aauth->get_user()->loc) $whr = ' AND (loc=' . $this->aauth->get_user()->loc . ') ';
    ...
    {
        $whr = '';
        if ($this->aauth->get_user()->loc) $whr = ' WHERE (i.loc=' . $this->aauth->get_user()->loc . ') ';
