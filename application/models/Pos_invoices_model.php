...
class Pos_invoices_model extends CI_Model
{
    ...
    public function customposinvoices($sdate, $edate)
    {
        $this->db->select('gi.id, gi.tid, gi.invoicedate, gi.invoiceduedate, gi.total, gi.status, gc.name');
        $this->db->from('geopos_invoices gi');
        $this->db->join('geopos_customers gc', 'gi.csd=gc.id', 'left');
        $this->db->where('DATE(gi.invoicedate) >=', datefordatabase($sdate));
        $this->db->where('DATE(gi.invoicedate) <=', datefordatabase($edate));
        if ($this->aauth->get_user()->loc) {
            $this->db->where('gi.loc', $this->aauth->get_user()->loc);
        }
        $query = $this->db->get();
        $result = $query->result();
        return $result; 
