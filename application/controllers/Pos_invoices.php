    ...
    public function action()
    {
        ...
        if ($ptype == 4) {
            ...
            if ($this->db->insert('geopos_invoices', $data)) {
                ...
                if ($pamnt > 0) $this->billing->paynow($invocieno, $pamnt, $tnote, $pmethod, $this->aauth->get_user()->loc);
...
// Your Account SID and Auth Token from twilio.com/console
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            $ammount += $invoices->total ;
           
        }
   
        //echo '<pre>'; print_r($ammount);exit;
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsFilteredotal" => $this->invocies->count_all($this->limited),
    ...
    public function ajax_list_pdf()
    {
        ...
        $records['data'] = $this->invocies->customposinvoices($sdate, $edate);
        $records['sdate'] = $sdate;
        $records['edate'] = $edate;

        $filename = $sdate .'_to_'.$edate.'_sales.pdf';
        $html = $this->load->view('pos/pdfinvoices', $records, true); 
...
//        if (INVV == 1) {
  //          $header = $this->load->view('print_files/invoice-header_v' . INVV, $data, true);
    //        $pdf = $this->pdf->load_split(array('margin_top' => 60));
      //      $pdf->SetHTMLHeader($header);
            //$pdf->SetHTMLHeader('<table style="background-color:yellow;"><tr><td>header</td></tr></table>');
        //}
       // if (INVV == 2) {
         //   $pdf = $this->pdf->load_split(array('margin_top' => 5));
       // }
        //$pdf->SetHTMLFooter('<div style="text-align: right;font-family: serif; font-size: 8pt; color: #5C5C5C; font-style: italic;margin-top:-6pt;">{PAGENO}/{nbpg} #' . $data['invoice']['tid'] . '</div>');
        $pdf->SetHTMLFooter('<table style=""><tr><td>&nbsp;</td></tr></table>');
       ...
       40, // margin bottom
        ...
        if ($this->input->get('d')) {
           ...
           // $pdf->Output('pdfs/pos_pdfs/'.$data['invoice']['tid']. '.pdf', 'F');
